...
class LineFunctions {
 ...
 public:
    static void lineFitting(int rows,
                            int cols,
    ...
    static void lineFittingSVD(cv::Point *points,
                               int length,
...
class PCAFunctions {
 public:
    ...
    void Ori_PCA(PointCloud<double> &cloud,
                 int k,
                 std::vector<PCAInfo> &pcaInfos,
                 double &scale,
                 double &magnitd);

    void PCASingle(std::vector<std::vector<double> > &pointData,
                   PCAInfo &pcaInfo);

    void MCMD_OutlierRemoval(std::vector<std::vector<double> > &pointData,
                             PCAInfo &pcaInfo);
