...
void LineFunctions::lineFitting(int rows,
                                int cols,
                                std::vector<cv::Point> &contour,
                                double thMinimalLineLength,
    ...
    subDivision(straightString, contour, 0, contour.size() - 1, minDeviation,
                int(thMinimalLineLength));
    ...
    for (int i = 0; i < straightString.size(); ++i) {
        ...
        lineFittingSVD(&straightString[i][0], straightString[i].size(),
                       parameters, maxDev);
        // if ( isOK )
        {
...
void LineFunctions::subDivision(
    std::vector<std::vector<cv::Point> > &straightString,
    std::vector<cv::Point> &contour,
    int first_index,
    int last_index,
    ...
    // Compute the length of the straight line segment defined by the endpoints
    // of the cluster.
    int x = first.x - last.x;
    int y = first.y - last.y;
    double length = sqrt(static_cast<double>((x * x) + (y * y)));

    // Find the pixels with maximum deviation from the line segment in order to
    // subdivide the cluster.
    int max_pixel_index = 0;
    double max_deviation = -1.0;

    for (int i = first_index, count = contour.size(); i != last_index;
         i = (i + 1) % count) {
        cv::Point current = contour[i];

        double deviation = static_cast<double>(
            abs(((current.x - first.x) * (first.y - last.y)) +
    ...
    //
    // 	// Compute the ratio between the length of the segment and the maximum
    // deviation.
    ...
    if ((max_deviation >= min_deviation) &&
        ((max_pixel_index - first_index + 1) >= half_min_size) &&
        ((last_index - max_pixel_index + 1) >= half_min_size)) {
        subDivision(straightString, contour, first_index, max_pixel_index,
                    min_deviation, min_size);
        subDivision(straightString, contour, max_pixel_index, last_index,
                    min_deviation, min_size);
    } else {
        //
        if (last_index - first_index > min_size) {
            ...
            // terminalIds.push_back(std::pair<int,int>(first_index,
            // last_index));
        }
    }
}

void LineFunctions::lineFittingSVD(cv::Point *points,
                                   int length,
...
void PCAFunctions::Ori_PCA(PointCloud<double> &cloud,
                           int k,
...
#pragma omp parallel for
    for (int i = 0; i < pointNum; ++i) {
        ...
        cv::Matx33d h_cov(h_cov_1, h_cov_2, h_cov_3, h_cov_2, h_cov_5, h_cov_6,
        ...
        double t = h_cov_evals.row(0).val[0] + h_cov_evals.row(1).val[0] +
                   h_cov_evals.row(2).val[0] + (rand() % 10 + 1) * MINVALUE;
    ...
    scale /= pointNum;
    magnitd =
        sqrt(cloud.pts[0].x * cloud.pts[0].x + cloud.pts[0].y * cloud.pts[0].y +
             cloud.pts[0].z * cloud.pts[0].z);
}

void PCAFunctions::PCASingle(std::vector<std::vector<double> > &pointData,
                             PCAInfo &pcaInfo) {
    int i, j;
    ...
    for (i = 0; i < k; ++i) {
        pcaInfo.idxIn[i] = i;
        h_mean +=
            cv::Matx31d(pointData[i][0], pointData[i][1], pointData[i][2]);
    }
    h_mean *= (1.0 / k);
    ...
    pcaInfo.lambda0 = h_cov_evals.row(2).val[0] /
                      (h_cov_evals.row(0).val[0] + h_cov_evals.row(1).val[0] +
...
void PCAFunctions::MCMD_OutlierRemoval(
    std::vector<std::vector<double> > &pointData, PCAInfo &pcaInfo) {
    double a = 1.4826;
    double thRz = 2.5;
    ...
    for (int j = 0; j < pcaInfo.idxIn.size(); ++j) {
        ...
        h_mean += cv::Matx31d(pointData[idx][0], pointData[idx][1],
                              pointData[idx][2]);
    }
...
double PCAFunctions::meadian(std::vector<double> dataset) {
    ...
    if (dataset.size() % 2 == 0) {
        ...
        return (dataset[dataset.size() / 2] + dataset[dataset.size() / 2 + 1]) /
               2.0;
    }
}
