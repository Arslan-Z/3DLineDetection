...
void LineDetection3D::run(PointCloud<double> &data,
                          int k,
...
void LineDetection3D::pointCloudSegmentation(
    std::vector<std::vector<int> > &regions) {
    cout << "----- Normal Calculation ..." << endl;
    PCAFunctions pcaer;
    pcaer.Ori_PCA(this->pointData, this->k, this->pcaInfos, this->scale,
                  this->magnitd);
...
void LineDetection3D::regionGrow(double thAngle,
                                 std::vector<std::vector<int> > &regions) {
    ...
    for (int i = 0; i < idx; ++i) {
        ...
        double xStrater = pointData.pts[idxStrater].x,
               yStrater = pointData.pts[idxStrater].y,
               zStrater = pointData.pts[idxStrater].z;
        ...
        while (count < clusterTemp.size()) {
            ...
            for (int j = 0; j < num; ++j) {
                ...
                double normalDev = abs(normalCur.val[0] * normalStarter.val[0] +
                                       normalCur.val[1] * normalStarter.val[1] +
                                       normalCur.val[2] * normalStarter.val[2]);
                // double normalDev = abs(normalCur.val[0] * normalSeed.val[0] +
                // normalCur.val[1] * normalSeed.val[1] + normalCur.val[2] *
                // normalSeed.val[2]);
                ...
                double dOrtho =
                    abs(dx * normalCur.val[0] + dy * normalCur.val[1] +
                        dz * normalCur.val[2]);
                if (dOrtho > thOrtho) {
                    continue;
                }
...
void LineDetection3D::regionMerging(double thAngle,
                                    std::vector<std::vector<int> > &regions) {
...
#pragma omp parallel for
    for (int i = 0; i < patches.size(); ++i) {
        ...
        for (int j = 0; j < pointAdjacentTemp.size(); ++j) {
            ...
            vector<int>::iterator new_end = unique(pointAdjacentTemp[j].begin(),
                                                   pointAdjacentTemp[j].end());
        ...
        if (!mergedIndex[i]) {
            ...
            while (count < patchIdx.size()) {
                ...
                for (int j = 0; j < patchAdjacent[idxSeed].size(); ++j) {
                    ...
                    devAngle = acos(normalStarter.val[0] * normalCur.val[0] +
                                    normalStarter.val[1] * normalCur.val[1] +
                                    normalStarter.val[2] * normalCur.val[2]);
                    // devDis = abs( normalSeed.val[0] * ptVector2.val[0] +
                    // normalSeed.val[1] * ptVector2.val[1] + normalSeed.val[2]
                    // * ptVector2.val[2] );
                    devDis = abs(normalStarter.val[0] * ptVector1.val[0] +
                                 normalStarter.val[1] * ptVector1.val[1] +
                                 normalStarter.val[2] * ptVector1.val[2]);
...
void LineDetection3D::planeBased3DLineDetection(
    std::vector<std::vector<int> > &regions, std::vector<PLANE> &planes) {
...
#pragma omp parallel for
    for (int i = 0; i < patches.size(); ++i) {
        ...
        cv::Mat_<double> planePt =
            (cv::Mat_<double>(3, 1) << patches[i].planePt.val[0],
             patches[i].planePt.val[1], patches[i].planePt.val[2]);
        cv::Mat_<double> normal =
            (cv::Mat_<double>(3, 1) << patches[i].normal.val[0],
             patches[i].normal.val[1], patches[i].normal.val[2]);

        for (int j = 0; j < patches[i].idxAll.size(); ++j) {
            int id = patches[i].idxAll[j];
            cv::Mat_<double> pt3d =
                (cv::Mat_<double>(3, 1) << pointData.pts[id].x,
                 pointData.pts[id].y, pointData.pts[id].z);
        ...
        std::sort(
            ptScales.begin(), ptScales.end(),
            [](const double &lhs, const double &rhs) { return lhs < rhs; });
        int idxNinety =
            min(int(double(ptScales.size()) * 0.9), int(ptScales.size() - 1));
        ...
        int margin = 0;
        cv::Mat mask;
        bool isok = maskFromPoint(pts2d, gridSideLength, xmin, ymin, xmax, ymax,
                                  margin, mask);
        if (!isok) {
            continue;
        }
        ...
        for (int m = 0; m < lines2d.size(); ++m) {
            ...
            for (int n = 0; n < lines2d[m].size(); ++n) {
                double length = abs(lines2d[m][n][1].x - lines2d[m][n][0].x) +
                                abs(lines2d[m][n][1].y - lines2d[m][n][0].y);
                if (length < thLineLengthPixel) {
                    continue;
                }

                lines2d[m][n][0].x =
                    (lines2d[m][n][0].x - margin) * gridSideLength + xmin;
                lines2d[m][n][0].y =
                    (lines2d[m][n][0].y - margin) * gridSideLength + ymin;

                lines2d[m][n][1].x =
                    (lines2d[m][n][1].x - margin) * gridSideLength + xmin;
                lines2d[m][n][1].y =
                    (lines2d[m][n][1].y - margin) * gridSideLength + ymin;
...
bool LineDetection3D::maskFromPoint(std::vector<cv::Point2d> &pts2d,
                                    double radius,
                                    double &xmin,
                                    double &ymin,
                                    double &xmax,
...
void LineDetection3D::lineFromMask(
    cv::Mat &mask,
    int thLineLengthPixel,
    ...
    cv::findContours(mask2, contours, hierarchy, CV_RETR_LIST,
    ...
    for (int i = 0; i < contours.size(); ++i) {
        ...
        LineFunctions::lineFitting(mask2.rows, mask2.cols, contours[i],
                                   thLength, lineTemp);
        lines.push_back(lineTemp);
    }
}

void LineDetection3D::postProcessing(
    std::vector<PLANE> &planes, std::vector<std::vector<cv::Point3d> > &lines) {
    ...
    for (int i = 0; i < planes.size(); ++i) {
        ...
        for (int m = 0; m < planes[i].lines3d.size(); ++m) {
            ...
            for (int n = 0; n < planes[i].lines3d[m].size(); ++n) {
                cv::Mat orientTemp = cv::Mat(planes[i].lines3d[m][n][1] -
                                             planes[i].lines3d[m][n][0]);
        ...
        for (int j = 0; j < lineInfos.size(); ++j) {
            ...
            if (!clusterInfos.size()) {
                clusterInfos.push_back(
                    std::pair<int, double>(clusterInfos.size(), length));
        ...
        if (clusterInfos.size() > 1) {
            ...
            if (LStruct < thNonStructPlaneRatio * totalLength ||
                LStruct < thStructPlane) {
                continue;
            }
        }
        ...
        for (int m = 0; m < planes[i].lines3d.size(); ++m) {
            ...
            if (numLines > 1) {
                ...
                if (idxOrthoPara.size()) {
                    // structural ratio
                    std::sort(idxOrthoPara.begin(), idxOrthoPara.end(),
...
void LineDetection3D::lineMerging(
    std::vector<PLANE> &planes, std::vector<std::vector<cv::Point3d> > &lines) {
    ...
    for (int i = 0; i < lineInfos.size(); ++i) {
        ...
        double vx0 = lineParas[id0][0], vy0 = lineParas[id0][1],
               vz0 = lineParas[id0][2];
        ...
        for (int j = 0; j < idHyps.size(); ++j) {
            ...
            double disOrtho1 =
                sqrt(vOrtho1.x * vOrtho1.x + vOrtho1.y * vOrtho1.y +
                     vOrtho1.z * vOrtho1.z);
            ...
            double disOrtho2 =
                sqrt(vOrtho2.x * vOrtho2.x + vOrtho2.y * vOrtho2.y +
                     vOrtho2.z * vOrtho2.z);

            if (disOrtho1 > thMergeRatio * lineScale ||
                disOrtho2 > thMergeRatio * lineScale) {
                continue;
            }

            // judge the overlapping ratio of two line
            cv::Point3d d1 = pts0 - pts1, d2 = pts0 - pte1, d3 = pte0 - pts1,
                        d4 = pte0 - pte1;
            ...
            if (gapRatio < 0.1 && gapLength < thGapRatio * lineScale) {
                ...
                if (gapRatio > 0) {
                    if (dis1 == disMerge) {
                        ...
                        lines[id0][1] =
                            pts0 - disNormal * cv::Point3d(vx0, vy0, vz0);
                        ...
                        lines[id0][0] =
                            pte0 - disNormal * cv::Point3d(vx0, vy0, vz0);
                    } else {
